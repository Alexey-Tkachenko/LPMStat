...
uint8_t SdFile::make83Name(const char* ptr, uint8_t* name) {
  uint8_t c;
 ...
 * Use SdFile::writeError to check for errors.
 */
size_t SdFile::write(const char* ptr) {
 ...
 * Use SdFile::writeError to check for errors.
 */
void SdFile::write_P(PGM_P ptr) {
  for (uint8_t c; (c = pgm_read_byte(ptr)); ptr++) write(c);
}
//------------------------------------------------------------------------------
/**
 ...
 * Use SdFile::writeError to check for errors.
 */
void SdFile::writeln_P(PGM_P ptr) {
  write_P(ptr);
  println();
}
#endif

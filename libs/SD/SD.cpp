...
boolean SDX_SDClass::begin(uint8_t csPin) {
  /*
...
boolean SDX_SDClass::begin(uint32_t clock, uint8_t csPin) {
  return card.init(SPI_HALF_SPEED, csPin) &&
...
SdFile SDX_SDClass::getParentDir(const char *filepath, int *index) {
...
SDX_File SDX_SDClass::open(const char *filepath, uint8_t mode) {
  /*
  ...
  return SDX_File(file, filepath);
}


/*
SDX_File SDClass::open(char *filepath, uint8_t mode) {
  //
...
boolean SDX_SDClass::exists(const char *filepath) {
  /*
...
boolean SDX_SDClass::mkdir(const char *filepath) {
  /*
...
boolean SDX_SDClass::rmdir(const char *filepath) {
  /*
...
boolean SDX_SDClass::remove(const char *filepath) {
  return walkPath(filepath, root, callback_remove);
}


// allows you to recurse into a directory
SDX_File SDX_File::openNextFile(uint8_t mode) {
  dir_t p;
...
void SDX_File::rewindDirectory(void) {  
  if (isDirectory())
    _file.rewind();
}
